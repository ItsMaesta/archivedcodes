...
if(!member) return this.client.yolla("Rol vermek istediğin kullanıcıyı belirt.", message.author, message.channel)
let maple = array.map(a => `\`${a["no"]}\` ${message.guild.roles.cache.get(a["rol"][0])}`).join("\n")

return message.channel.send(embed1.setDescription(`\`\`\`(${member.user.username.replace(/\`/g, "")} - ${member.user.id}) kullanıcısına verilicek rolün numarasını yazınız.\`\`\`
    ...
    array.forEach(a => {
            if(a.no == collected.first().content) {
                if(Array.isArray(a.rol)) {
                    a.rol.forEach(slm=>{
                        member.roles.add(slm)
                    })
                } 
                  this.client.yolla(`${member} adlı kullanıcıya <@&${a.rol[0]}> rolü başarıyla verildi.`, message.author, message.channel)

log.send(embed1.setDescription(`${member}-\`(${member.id})\` adlı kullanıcıya ${message.member} tarafından yetki verildi. 
───────────────
\`•\` Kullanıcıya verilen yetki : <@&${a.rol[0]}>
`))
