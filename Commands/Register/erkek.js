...
class erkek extends Command {
    ...
    if (member.roles.cache.has("936326075632459807"))  return this.client.yolla("Bu kullanıcı zaten sunucuda kayıtlı.", message.author, message.channel);
...
• Voice Mute Sayısı : `+voiceMute+`\`\`\``)
    ...
    await member.roles.add("936326075632459807");///erkek rol
    await member.roles.remove("936326074676150333");///kadın rol 
    ...
    const embed1 = new MessageEmbed()
      ...
      .setColor("RANDOM")
    .setDescription(`
${member.toString()}-(\`${member.id}\`) Kullanıcısı <@&936326075632459807> rolü ile kaydedildi.
`);
    ...
    .setAuthor(message.author.tag, message.author.avatarURL({ dynamic: true }))
    .setDescription(`${member.toString()}-(\`${member.id}\`) Adlı kullanıcı <@&936326075632459807> rolü ile kaydedildi.
    ───────────────
    ...
    isimler.findOne({ user: member.id }, async (err, res) => {
                ...
                if (!res) {
                    let arr = []
                    arr.push({ isim: `${isim} | ${yaş}`, state: "<@&936326075632459807>", yetkili: message.author.id })
                    ...
                    res.isimler.push({ isim: `${isim} | ${yaş}`, state: "<@&936326075632459807>", yetkili: message.author.id })
                    res.save().catch(e => console.log(e))
                }
            })
            })
