...
class Ban extends Command {
    ...
    async run(message, args, perm) {
        ...
        if (!user) return this.client.yolla("Bu kullanıcı geçerli değil.", message.author, message.channel)
        let reason = args.slice(1).join(" ") || "Sebep Belirtilmedi."
        let count = await data.countDocuments().exec();
        let can = count
        const embed = new Discord.MessageEmbed()
        .setAuthor(message.guild.name, message.guild.iconURL({dynamic: true}))
        .setImage("https://i.pinimg.com/originals/89/5f/cc/895fcc2618a5681c2ab76c828439e4a2.gif")
        .setDescription(`\`${user.tag}\` Adlı kullanıcı sunucudan yasaklandı.`)
        message.guild.fetchBans(true).then(async (bans) => {
            ...
            if (!ban) {
                ...
                if (banNum == 5) return this.client.yolla("Komut kullanma sınırına ulaştın.", message.author, message.channel)
